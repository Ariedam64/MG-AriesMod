// Visual atoms for the Skins menu.
//
// Deliberately self-contained rather than reusing the editor menu's helpers:
// those styles are only injected when the editor tab is opened, so borrowing
// its class names would leave Skins unstyled for anyone who never opens it.

const STYLE_ID = 'qws-skins-menu-css';
...
.qws-sk-cell.is-active { border-color: ${TEAL_BORDER}; background: ${TEAL_DIM}; }
...
.qws-sk-toggle input:checked ~ .qws-sk-track .qws-sk-thumb { transform:translateX(16px); background:${TEAL}; }
...
.qws-sk-input:focus { border-color: ${TEAL_BORDER}; }
.qws-sk-input option { background: #10151c; color: ${TEXT}; }
`;
